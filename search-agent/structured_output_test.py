...
class AgentResponse(BaseModel):
    ...
    sources: List[Source] = Field(
        default_factory=list, description="List of soucres used by agent for response"
    )
    ...
    result = agent.invoke(
        {
            "messages": HumanMessage(
                content="Search for 3 SDET job openings in Hyderabad area with 12+ years of experience"
            )
        }
    )
    print(result)
